...
class VideoGeneratorCLI:
    ...
    def create_argument_parser() -> argparse.ArgumentParser:
        ...
        parser.add_argument('--model', type=str, choices=allowed_models,
                          default='openai/gpt-4o-mini', help='AI model to use')
        ...
        # Enhanced RAG flags
        parser.add_argument('--use_enhanced_rag', action='store_true', default=True, help='Use enhanced RAG components')
        parser.add_argument('--disable_enhanced_rag', action='store_true', help='Disable enhanced RAG components')
        parser.add_argument('--enable_rag_caching', action='store_true', default=True, help='Enable RAG caching')
        parser.add_argument('--enable_quality_monitoring', action='store_true', default=True, help='Enable RAG quality monitoring')
        parser.add_argument('--enable_error_handling', action='store_true', default=True, help='Enable enhanced error handling')
        parser.add_argument('--rag_cache_ttl', type=int, default=3600, help='RAG cache TTL in seconds')
        parser.add_argument('--rag_performance_threshold', type=float, default=2.0, help='RAG performance threshold in seconds')
    ...
    def create_config_from_args(args) -> VideoGenerationConfig:
        ...
        # Handle enhanced RAG flag logic
        use_enhanced_rag = args.use_enhanced_rag and not args.disable_enhanced_rag if hasattr(args, 'disable_enhanced_rag') else getattr(args, 'use_enhanced_rag', True)
        ...
        return VideoGenerationConfig(
            ...
            enable_rag_caching=getattr(args, 'enable_rag_caching', True),
            enable_quality_monitoring=getattr(args, 'enable_quality_monitoring', True),
            enable_error_handling=getattr(args, 'enable_error_handling', True),
            rag_cache_ttl=getattr(args, 'rag_cache_ttl', 3600),
            rag_performance_threshold=getattr(args, 'rag_performance_threshold', 2.0)
        )
