...
def main():
    ...
    # Initialize separate models with configuration manager support
    ...
    try:
        ...
        text_model_config = config_manager.get_model_config(args.model_text)
        video_model_config = config_manager.get_model_config(args.model_video)
        image_model_config = config_manager.get_model_config(args.model_image)
        ...
        text_model = LiteLLMWrapper(
            ...
            timeout=text_model_config.get('timeout'),
            max_retries=text_model_config.get('max_retries')
        )
        ...
        image_model = LiteLLMWrapper(
            ...
            temperature=0.0,
            api_key=image_model_config.get('api_key'),
            base_url=image_model_config.get('base_url'),
            timeout=image_model_config.get('timeout'),
            max_retries=image_model_config.get('max_retries')
        )
    except Exception as e:
        print(f"Warning: Could not load model configurations: {e}")
